...
namespace WebApiSO.Features.Supplies
{
    public class GetSuppliesHandler : IServiceHandler<Pagination, IEnumerable<CustomSupplyDto>>
    {
        ...
        public async Task<Result<IEnumerable<CustomSupplyDto>>> Handle(Pagination pagination)
        {
            ...
            var states = repository.Entity<ServiceOrderTaskState>();
            ...
            foreach (var item in result)
            {
                ...
                item.ServiceOrderTask.ServiceOrderTaskState = ServiceOrderTaskStateDto.ToDto(await states.FirstOrDefaultAsync(so => so.Id == item.ServiceOrderTask.ServiceOrderTaskStateId));
                newList.Add(item);
            }
